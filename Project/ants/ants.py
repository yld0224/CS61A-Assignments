...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            ori_ant = place.ant
            if ori_ant.can_contain(self):
                ori_ant.ant_contained = self
            elif self.can_contain(ori_ant):
                self.ant_contained = ori_ant
                place.ant = self
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while cur_place.is_hive == False:
            victim = random_bee(cur_place.bees)
            if victim != None and distance >= self.lower_bound and distance <= self.upper_bound:
                return victim
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if amount >= self.health:
            bonus = self.damage
        vic_bees = list(self.place.bees)#copy it
        for bee in vic_bees:
            bee.reduce_health(amount + bonus) 
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while nxt_place != None:
            brave = nxt_place.ant
            if brave != None:
                if brave.has_doubled == False:
                    brave.has_doubled = True
                    brave.damage *=2
                if brave.is_container:
                    if brave.ant_contained != None:
                        if brave.ant_contained.has_doubled == False:
                            brave.ant_contained.has_doubled = True
                            brave.ant_contained.damage *= 2
